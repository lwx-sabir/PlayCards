...
| `StorePurchaseButton.cs` | port of WGWB's `ShopIapPurchaseButton`: `[SerializeField] string productId`, title/price/amount/bonus `TMP_Text`s, loading/unavailable roots, `canInteract = IsReady && HasFetchedProduct && !Unavailable && !Processing`; price from the store, amount/badge from the catalog; `ButtonSound`/`ButtonJuice`/`Haptic` beside it. **Two badge slots** — `badgeTexts`/`badgeRoot` ← `Badge` (the ribbon, "2X VALUE") and `badge2Texts`/`badge2Root` ← `Badge2` (the corner hex, "POPULAR"): independent marks (price vs. other players), so they are separate catalog fields. Sale slots per §3.2. Reza places it on his cards — the Shop screen itself is his (`iap-shop-design`) |
| `StoreRewardLines.cs` | the reward ROWS of a bundle card — one per `Lines` entry (`2,000,000` / `Kash 50` / `VIP 500`), because how many rows is the CATALOG's decision, not the prefab's (adding Gems to a bundle must not need a client build). Reuses `PlayCard.UI.BundleRewardView` as the row (icon + amount, the daily bundle's) and the `rewardId → sprite` list `RewardFly` uses; pools rows rather than destroying them; shows the **sale's boosted amounts** during a value bonus; `maxRows`, `hideXp`, and optional `summaryTexts` for a one-line variant. `StorePurchaseButton` still owns the single headline amount for a chip/Kash pack |
