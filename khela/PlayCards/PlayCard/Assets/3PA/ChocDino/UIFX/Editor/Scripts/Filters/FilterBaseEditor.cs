...
namespace ChocDino.UIFX.Editor
{
	internal class FilterBaseEditor : BaseEditor
	{
		...
		{
			_isTextMeshPro = IsTargetsContainsTextMeshPro();
		}

		private bool IsTargetsContainsTextMeshPro()
		{
			foreach (FilterBase filter in this.targets)
			{
				...
				if (graphic)
				{
					...
					// Also check the base type in case the component inherits it
					var baseType = graphicType.BaseType;
					if (graphicType.ToString().Contains(TextMeshProGraphicTypeName) || baseType.ToString().Contains(TextMeshProGraphicTypeName))
					{
						return true;
					}
				}
		...
		protected void DrawFilterRenderSpaceControl(SerializedProperty propRenderSpace)
		{
			if (!_isTextMeshPro)
			{
				EnumAsToolbarCompact(propRenderSpace);
			}
			else
			{
				EditorGUILayout.HelpBox("Render Space property is controlled via the FilterStack component.", MessageType.Info, true);
				EditorGUI.BeginDisabledGroup(_isTextMeshPro);
				EnumAsToolbarCompact(propRenderSpace);
		...
		private bool CanBakeFiltersToImage(FilterBase filter)
		{
			...
			// Also check the base type in case the component inherits it
			if (graphic.GetType().BaseType.ToString().Contains(TextMeshProGraphicTypeName)) { return false; }
		...
		internal void OnInspectorGUI_CheckResolution(FilterBase filter)
		{
			...
			if (graphic)
			{
				...
				if (canvas)
				{
					...
					if (canvas.pixelRect.width < minSize || canvas.pixelRect.height < minSize)
					{
						EditorGUILayout.HelpBox(string.Format("Low Game window resolution {0}x{1} can affect UIFX rendering resolution in Scene window.", canvas.pixelRect.width, canvas.pixelRect.height), MessageType.Warning, true);
						EditorGUILayout.Space();
					}
				}
			}
		...
		internal bool OnInspectorGUI_Check(FilterBase filter)
		{
			...
			if (graphic)
			{
				...
				foreach (var obj in this.targets)
				{
					...
					{
						var graphicType = filterInstance.GraphicComponent.GetType();
						// Also check the base type in case the component inherits it
						var baseType = graphicType.BaseType;
						if (graphicType.ToString().Contains(TextMeshProGraphicTypeName) || baseType.ToString().Contains(TextMeshProGraphicTypeName))
						{
				...
				OnInspectorGUI_CheckResolution(filter);
			}
			return result;
		}
