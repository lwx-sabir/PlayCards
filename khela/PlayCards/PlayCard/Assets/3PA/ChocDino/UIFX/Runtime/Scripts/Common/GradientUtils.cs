...
namespace ChocDino.UIFX
{
	...
	internal class GradientTexture : System.IDisposable
	{
		...
		private readonly bool _premultiplyAlpha = false;
		...
		public bool PremultiplyAlpha { get => _premultiplyAlpha; }
		public Texture2D Texture { get { return _texture; } }

		public static void Create(ref GradientTexture gradientTexture, int resolution, bool premultiplyAlpha = false)
		{
			if (gradientTexture != null && (gradientTexture.Resolution != resolution || gradientTexture.PremultiplyAlpha != premultiplyAlpha))
			{
				ObjectHelper.Dispose(ref gradientTexture);
			}
			if (gradientTexture == null)
			{
				gradientTexture = new GradientTexture(resolution, premultiplyAlpha);
			}
		}

		public GradientTexture(int resolution, bool premultiplyAlpha = false)
		{
		...
		public void Update(Gradient gradient)
		{
			...
			if (_texture == null)
			{
				...
				// Note we don't use TextureCreationFlags.DontUploadUponCreate as this was causing a crash in Unity 6.2.15 in prefab mode when using a 
				// floating-point texture.  When using a non-FP texture it would error with "kTextureUploadUninitialized is only allowed for newly created textures".
				// So I guess this flag is only useful for textures that are never updated after creation.
				_texture = new Texture2D(_resolution, 1, s_gradientTextureFormat, TextureCreationFlags.DontInitializePixels);
#else
			...
			if (isDirty)
			{
				...
				if (_premultiplyAlpha)
				{
					for (int x = 0; x < _resolution; x++)
					{
						_colors[x].r *= _colors[x].a;
						_colors[x].g *= _colors[x].a;
						_colors[x].b *= _colors[x].a;
					}
				}
		...
		public void Update(AnimationCurve curve)
		{
			if (_texture == null)
			{
				...
				// floating-point texture.  When using a non-FP texture it would error with "kTextureUploadUninitialized is only allowed for newly created textures".
				// So I guess this flag is only useful for textures that are never updated after creation.
				_texture = new Texture2D(_resolution, 1, s_curveTextureFormat, TextureCreationFlags.DontInitializePixels);
#else
