...
namespace Invector.vCharacterController
{
    ...
    public class vShooterMeleeInput : vMeleeCombatInput, vIShooterIKController, PlayerController.vILockCamera
    {
        ...
        [System.NonSerialized] public vArmAimAlign leftArmAim, rightArmAim;
        ...
        public virtual vControlAimCanvas controlAimCanvas
        {
            get
            {
                ...
                {
                    _controlAimCanvas = FindAnyObjectByType<vControlAimCanvas>();
                    if (_controlAimCanvas)
                    {
        ...
        public override void UpdateCameraStates()
        {
            ...
            if (tpCamera == null)
            {
                tpCamera = FindAnyObjectByType<vCamera.vThirdPersonCamera>();
                if (tpCamera == null)
                {
                    return;
                }
        ...
        {
            bool _pose = cc.IsAnimatorTag("Upperbody Pose");
            bool _can = armAlignmentWeight >= 0.5f && _pose && shootCountA > 0 && !isReloading;
            if (shotInput.GetButton())   // TEMP DEBUG — remove when done
                Debug.Log("[CanDoShots] can=" + _can
                    + " | armAlignmentWeight=" + armAlignmentWeight + " (>=0.5? " + (armAlignmentWeight >= 0.5f) + ")"
                    + " | UpperbodyPose=" + _pose
                    + " | shootCountA=" + shootCountA
