...
namespace ChocDino.UIFX
{
	...
	public abstract partial class FilterBase : UIBehaviour, IMaterialModifier, IMeshModifier, IClippable
	{
		...
		public virtual bool DoParametersHideSource()
		{
			return false;
		}
		...
		public virtual Material GetModifiedMaterial(Material baseMaterial)
		{
			...
			if (GraphicComponent != null && CanApplyFilter())
			{
				if (baseMaterial && baseMaterial.shader)
				{
					...
					{
						if (baseMaterial != Graphic.defaultGraphicMaterial && baseMaterial.shader != Graphic.defaultGraphicMaterial.shader)
						{
							// NOTE: getting the shader name generates garbage, so we cache the material to minimise this
							_isLastMaterialTMPro = (baseMaterial.shader.name.StartsWith(TextMeshProShaderPrefix, System.StringComparison.Ordinal));
							if (!_isLastMaterialTMPro) { _isLastMaterialSpine = (baseMaterial.shader.name.StartsWith(SpineShaderPrefix, System.StringComparison.Ordinal)); }
						}
						else
						{
					...
					if (CanSelfRender() && !DoParametersHideSource())
					{
		...
		protected override void OnValidate()
		{
			...
			// In older versions of Unity:
			// OnValidate is also called when a script file is saved outside the editor, and it causes
			// material variables that don't have properties to be lost, so Canvas.ForceUpdateCanvases() fixes this.
			// It *seems* we can check for this by checking EditorApplication.isCompiling
		...
		private bool _isChangingMultipleProperties;
		private bool _isPropertyChanged;

		public void BeginPropertyChange()
		{
			Debug.Assert(!_isChangingMultipleProperties);
			_isChangingMultipleProperties = true;
			_isPropertyChanged = false;
		}
		public void EndPropertyChange()
		{
			Debug.Assert(_isChangingMultipleProperties);
			_isChangingMultipleProperties = false;
		...
		{
			if (_isChangingMultipleProperties)
			{
			...
			else
			{
				...
				bool needsVerticesUpdate = (_isFilterEnabled != CanApplyFilter()) || (DoParametersHideSource() && _isFilterEnabled) || (_lastRenderAlpha != GetAlpha());
		...
		#region Warnings And Error Reporting
		...
		private static bool s_warnWorldSpaceCanvasResolution = false;
		private static bool s_warnWorldSpaceCanvasScale = false;
		private static bool s_warnCanvasCameraLayer = false;

		#endif

		void DetectCanvasProblems()
		{
			#if UNITY_EDITOR
			if (_canvas != null)
			{
				...
				{
					// Detect if the world-space canvas is too small.
					if (!s_warnWorldSpaceCanvasResolution)
					{
						#if UNITY_2020_2_OR_NEWER
						Vector2 renderSize = rootCanvas.renderingDisplaySize;
						#else
						Vector2 renderSize = rootCanvas.pixelRect.size;
						#endif
						if (renderSize.x < 800 || renderSize.y < 600)
						{
							Debug.LogWarning("[UIFX] Canvas '" + rootCanvas.name  + "' render resolution " + renderSize + " seems low for world-space rendering and can cause problems with UIFX rendering.");
							s_warnWorldSpaceCanvasResolution = true;
						}
					}
					// Detect if the world-space transform scale is too large.
					if (!s_warnWorldSpaceCanvasScale)
					{
						if (rootCanvas.transform.localScale.x > 0.5f || rootCanvas.transform.localScale.y > 0.5f || rootCanvas.transform.localScale.z > 0.5f)
						{
							Debug.LogWarning("[UIFX] Canvas '" + rootCanvas.name + "' scale " + rootCanvas.transform.localScale + " seems too high for world-space rendering and can cause problems with UIFX rendering.");
							s_warnWorldSpaceCanvasScale = true;
						}
					}
				}
				// Detect if the canvas camera cullingMask can actually render this UI item.
				if (!s_warnCanvasCameraLayer)
				{
					if (rootCanvas.renderMode != RenderMode.ScreenSpaceOverlay && _renderSpace == FilterRenderSpace.Screen)
					{
						var camera = GetRenderCamera();
						if (camera != null)
						{
							// Detect if the assigned camera can even render this UI..
							bool canCameraRenderThis = (camera.cullingMask & (1 << this.gameObject.layer)) != 0;
							if (!canCameraRenderThis)
							{
								Debug.LogWarning("[UIFX] Canvas '" + rootCanvas.name + "' worldCamera is set to camera '" + camera.name + "' which doesn't contain the layer '" + LayerMask.LayerToName(this.gameObject.layer) + "' in it's cullingMask.");
								s_warnCanvasCameraLayer = true;
							}
						}
					}
				}
		...
		private List<Color> _meshColors;
		private List<Vector3> _meshNormals;
		private List<Vector4> _meshTangents;
		// From Unity 2020.2 UIVertex.uv* changed from Vector2 to Vector4
		#if UNITY_2020_2_OR_NEWER
		private List<Vector4> _meshUV0;
		private List<Vector4> _meshUV1;
		private List<Vector4> _meshUV2;
		private List<Vector4> _meshUV3;
		#else
		private List<Vector2> _meshUV0;
		private List<Vector2> _meshUV1;
		private List<Vector2> _meshUV2;
		private List<Vector2> _meshUV3;
		#endif
		...
		private bool HasMeshChanged(VertexHelper verts)
		{
			...
			if (_meshVertices != null) { _meshVertices.Clear(); } else { _meshVertices = new List<Vector3>(4); } _mesh.GetVertices(_meshVertices);
			if (_meshColors != null) { _meshColors.Clear(); } else { _meshColors = new List<Color>(4); }
			#if UNITY_2020_2_OR_NEWER
			if (_meshUV0 != null) { _meshUV0.Clear(); } else { _meshUV0 = new List<Vector4>(4); }
			#else
			if (_meshUV0 != null) { _meshUV0.Clear(); } else { _meshUV0 = new List<Vector2>(4); }
			#endif

			_mesh.GetVertices(_meshVertices);
			_mesh.GetColors(_meshColors);
			_mesh.GetUVs(0, _meshUV0);
			...
			for (int i = 0; i < vertexCount; i++)
			{
				...
				if (s_vertex.position != _meshVertices[i])
				{
					return true;
				}
				if (s_vertex.color != _meshColors[i])
				{
					return true;
				}
				if (s_vertex.uv0 != _meshUV0[i])
				{
					return true;
				}
			}

			bool checkUV1 = ((_canvas.additionalShaderChannels & AdditionalCanvasShaderChannels.TexCoord1) != 0);
			bool checkUV2 = ((_canvas.additionalShaderChannels & AdditionalCanvasShaderChannels.TexCoord2) != 0);
			bool checkUV3 = ((_canvas.additionalShaderChannels & AdditionalCanvasShaderChannels.TexCoord3) != 0);
			bool checkNormal = ((_canvas.additionalShaderChannels & AdditionalCanvasShaderChannels.Normal) != 0);
			bool checkTangent = ((_canvas.additionalShaderChannels & AdditionalCanvasShaderChannels.Tangent) != 0);

			if (checkUV1 || checkUV2 || checkUV3 || checkNormal || checkTangent)
			{
				#if UNITY_2020_2_OR_NEWER
				if (checkUV1) { if (_meshUV1 != null) { _meshUV1.Clear(); } else { _meshUV1 = new List<Vector4>(4); } _mesh.GetUVs(1, _meshUV1); }
				if (checkUV2) { if (_meshUV2 != null) { _meshUV2.Clear(); } else { _meshUV2 = new List<Vector4>(4); } _mesh.GetUVs(2, _meshUV2); }
				if (checkUV3) { if (_meshUV3 != null) { _meshUV3.Clear(); } else { _meshUV3 = new List<Vector4>(4); } _mesh.GetUVs(3, _meshUV3); }
				#else
				if (checkUV1) { if (_meshUV1 != null) { _meshUV1.Clear(); } else { _meshUV1 = new List<Vector2>(4); } _mesh.GetUVs(1, _meshUV1); }
				if (checkUV2) { if (_meshUV2 != null) { _meshUV2.Clear(); } else { _meshUV2 = new List<Vector2>(4); } _mesh.GetUVs(2, _meshUV2); }
				if (checkUV3) { if (_meshUV3 != null) { _meshUV3.Clear(); } else { _meshUV3 = new List<Vector2>(4); } _mesh.GetUVs(3, _meshUV3); }
				#endif
				if (checkNormal) { if (_meshNormals != null) { _meshNormals.Clear(); } else { _meshNormals = new List<Vector3>(4); } _mesh.GetNormals(_meshNormals); }
				if (checkTangent) { if (_meshTangents != null) { _meshTangents.Clear(); } else { _meshTangents = new List<Vector4>(4); } _mesh.GetTangents(_meshTangents); }

				for (int i = 0; i < vertexCount; i++)
				{
					verts.PopulateUIVertex(ref s_vertex, i);

					if (checkUV1 && s_vertex.uv1 != _meshUV1[i])
					{
						return true;
					}
					if (checkUV2 && s_vertex.uv2 != _meshUV2[i])
					{
						return true;
					}
					if (checkUV3 && s_vertex.uv3 != _meshUV3[i])
					{
						return true;
					}
					if (checkNormal && s_vertex.normal != _meshNormals[i])
					{
						return true;
					}
					if (checkTangent && s_vertex.tangent != _meshTangents[i])
					{
						return true;
					}
				}
		...
		public void ModifyMesh(VertexHelper verts)
		{
			...
			_lastRenderAdjustLeftDown = _lastRenderAdjustRightUp = Vector2Int.zero;
			_isResolveTextureDirty = true;
			if (CanApplyFilter() && CanSelfRender() && !DoParametersHideSource())
			{
				_isFilterEnabled = true;
			...
			else if (DoParametersHideSource())
			{
				// Filter makes source invisible
				verts.Clear();
			}
			else
			{
		...
		protected bool IsCanvasPixelPerfect()
		{
			if (_canvas)
			{
				return _canvas.pixelPerfect && _canvas.renderMode != RenderMode.WorldSpace && _canvas.scaleFactor != 0f;
			}
			return false;
		}

		private bool IsRenderedScreenSizeAPixel(Vector3 localMin, Vector3 localMax, Camera renderCamera)
		{
			Debug.Assert(_canvas);
			Debug.Assert(_canvas.pixelPerfect);
			Debug.Assert(_canvas.renderMode != RenderMode.WorldSpace);

			// Transform to world-space
			// We only apply the local scale transform - this is because applying position wouldn't have any effect, and more importantly,
			// applying rotation makes it not possible to determine the true scale of the screen pixel area.
			Matrix4x4 scale = Matrix4x4.Scale(this.transform.lossyScale);
			Vector3 worldMin, worldMax;
			if (_canvas.renderMode == RenderMode.ScreenSpaceOverlay)
			{
				Debug.Log(localMax.y - localMin.y);
				// It's not the true world coordinate, but it's good enough for this calculation.
				worldMin = scale.MultiplyPoint3x4(localMin);
				worldMax = scale.MultiplyPoint3x4(localMax);
			}
			else
			{
				// NOTE: The inverse Canvas scale is applied because we HAVE to multiply by the canvas matrix to transform to world space
				// but we need to exclude the canvas scale because in RenderMode.ScreenSpaceCamera the Camera FOV/OrtoSize affects scaling,
				// and scale transform.lossyScale already includes the Canvas scale so we don't want it applied twice.
				// Also we have to multiply by the Canvas matrix, because if the Camera has rotation and the Canvas also has this rotation
				// which gets the point in the same space as the camera.
				Matrix4x4 canvasScale = Matrix4x4.Scale(_canvas.transform.localScale);
				Matrix4x4 canvasScaleInv = canvasScale;
				Matrix4x4.Inverse3DAffine(canvasScale, ref canvasScaleInv);

				scale *= canvasScaleInv;
				Vector3 scaledLocalMin = scale.MultiplyPoint3x4(localMin);
				Vector3 scaledLocalMax = scale.MultiplyPoint3x4(localMax);
				worldMin = _canvas.transform.TransformPoint(scaledLocalMin);
				worldMax = _canvas.transform.TransformPoint(scaledLocalMax);
			}

			// Transform to screen-space
			Vector3 screenMin = renderCamera.WorldToScreenPoint(worldMin);
			Vector3 screenMax = renderCamera.WorldToScreenPoint(worldMax);

			//Debug.Log("min " + screenMin.x + " " + screenMin.y + " " + screenMin.z);
			//Debug.Log("max " + screenMax.x + " " + screenMax.y + " " + screenMax.z);

			// Get the pixel area
			// Note: we had to add the divisison by Canvas.scaleFactor which shouldn't be necessary, but see notes below.
			// At normal scales having this division gets us incorrect results, but at tiny scales it's fine.
			float screenWidth = Mathf.Abs(screenMax.x - screenMin.x) / _canvas.scaleFactor;
			float screenHeight = Mathf.Abs(screenMax.y - screenMin.y) / _canvas.scaleFactor;
			screenWidth = Mathf.Ceil(screenWidth);
			screenHeight = Mathf.Ceil(screenHeight);
			//Debug.Log("screen: " + screenWidth + " x " + screenHeight);

			// NOTE: There is a bug in Unity's implementation of (UUM-103165) where it doesn't correctly take into account 
			// Canvas.scaleFactor, which can result in the incorrect size being reported here.
			// For example, set CanvasScaler.referenceResolution to 1080p, and the GameView to 4K.. then no matter how small
			// the scale is, the snapped pixels will snap to a height of 2px instead of the expected 4px.
			return (screenWidth <= 1f) || (screenHeight <= 1f);
		}

		protected bool IsPixelPerfectCorrection()
		{
			// NOTE: In later versions of Unity they added this change:
			// uGUI: Ensure Pixel Perfect Rects are not rounded to zero width/height (UUM-103165)
			// This affect these Unity versions:
			// 6000.3.0b7 onwards
			// 6000.2.1 onwards
			// 6000.1.* not present
			// 6000.0.56 onwards
			//
			// The result is that when Canvas.pixelPerfect is enabled, and a Graphic matrix has a VERY tiny scale (eg 0.0000001)
			// this new change will MASSIVELY scale up the Graphic vertices so that when it is subsequently scaled down
			// during rendering, it will snap to a single pixel.  The problem for UIFX is that in Canvas Renderspace mode we use
			// the mesh geometry to determine the size of our RenderTextures and this results in HUGE textures.
			//
			// So we try to detect these conditions and force RenderSpace.Screen to prevent huge texture allocations.
			#if (!UNITY_6000_0_56_OR_NEWER || UNITY_6000_2_0 || (UNITY_6000_1_OR_NEWER && !UNITY_6000_2_1_OR_NEWER))
			// These versions don't require any correction.
			#else
			if (_renderSpace == FilterRenderSpace.Canvas)
			{
				if (IsCanvasPixelPerfect())
				{
					//if (this.transform.lossyScale.x <= 0.001f || this.transform.lossyScale.y <= 0.001f)
					{
						// Get the local bounds (post pixelPerfect snapping)
						Vector3 localMin = Vector3.zero;
						Vector3 localMax = Vector3.zero;
						if (_sourceArea == FilterSourceArea.Geometry && GraphicComponent && _mesh)
						{
							localMin = _mesh.bounds.min;
							localMax = _mesh.bounds.max;
						}
						else if (RectTransformComponent)
						{
							var rect = RectTransformUtility.PixelAdjustRect(RectTransformComponent, _canvas);
							localMin = new Vector3(rect.xMin, rect.yMin, 0f);
							localMax = new Vector3(rect.xMax, rect.yMax, 0f);
						}

						// Scale the local bounds by the localToWorld scale.
						float width = ((localMax.x - localMin.x) * this.transform.lossyScale.x) / 1f;
						float height = ((localMax.y - localMin.y) * this.transform.lossyScale.y) / 1f;

						//Debug.Log(_canvas.scaleFactor);
						//Debug.Log("height " + width + " x " + height);

						// Do more extensive test if the dimession is less than one pixel.
						// NOTE: it should be <= 1.0 but because the new snapping doesns't account for scaleFactor we have to do this.
						if (width <= _canvas.scaleFactor || height <= _canvas.scaleFactor)
						{
							Camera cam = _canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : _canvas.worldCamera;
							return IsRenderedScreenSizeAPixel(localMin, localMax, cam);
						}
					}
				}
			}
		...
		protected bool GenerateScreenRect()
		{
			...
			DetectCanvasProblems();
			GetResolutionScalingFactor();
			bool isPixelPerfectCorrection = IsPixelPerfectCorrection();

			bool isRenderSpaceCanvas = _renderSpace == FilterRenderSpace.Canvas;

			// Calculate screen area
			{
				// TODO: only recalculate when mesh/camera changes
				Camera renderCamera = GetRenderCamera();
				_screenRect.Start(isRenderSpaceCanvas ? null : renderCamera, _renderSpace);
				if (!isPixelPerfectCorrection)
				{
					if (_sourceArea == FilterSourceArea.Geometry && GraphicComponent)
					{
						_screenRect.AddMeshBounds(isRenderSpaceCanvas ? null : this.transform, _mesh);
					}
					else if (RectTransformComponent)
					{
						_screenRect.AddRect(isRenderSpaceCanvas ? null : this.transform, RectTransformComponent.rect);
					}
				}
				_screenRect.End();
		...
		protected Rect GetScreenRect()
		{
			if (_canvas != null)
			{
				...
				r = new Rect(Mathf.FloorToInt(r.xMin), Mathf.FloorToInt(r.yMin), Mathf.CeilToInt(r.xMax) - Mathf.FloorToInt(r.xMin), Mathf.CeilToInt(r.yMax) - Mathf.FloorToInt(r.yMin));
				r.x /= sw;
				r.y /= sh;
				r.width /= sw;
				r.height /= sh;
