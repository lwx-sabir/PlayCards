...
namespace ChocDino.UIFX
{
	...
	public class ScreenRectFromMeshes
	{
		...
		public void End()
		{
			...
			_meshRect = _screenRect = Rect.MinMaxRect(minScreen.x, minScreen.y, maxScreen.x, maxScreen.y);
		}
		...
		public void Adjust(Vector2Int leftDown, Vector2Int rightUp)
		{
			...
			Debug.Assert(Mathf.Abs((long)leftDown.x) < 8192 && Mathf.Abs((long)leftDown.y) < 8192, string.Format("[UIFX] Out of range adjustment {0}-{1}", leftDown, rightUp));
			Debug.Assert(Mathf.Abs((long)rightUp.x) < 8192 && Mathf.Abs((long)rightUp.y) < 8192, string.Format("[UIFX] Out of range adjustment {0}-{1}", leftDown, rightUp));
		...
		public void BuildScreenQuad(Camera camera, Transform xform, Color32 color, VertexHelper vh)
		{
			...
			if (xform)
			{
				...
				v0 = worldToLocal.MultiplyPoint3x4(v0);
				v1 = worldToLocal.MultiplyPoint3x4(v1);
				v2 = worldToLocal.MultiplyPoint3x4(v2);
				v3 = worldToLocal.MultiplyPoint3x4(v3);
			}
