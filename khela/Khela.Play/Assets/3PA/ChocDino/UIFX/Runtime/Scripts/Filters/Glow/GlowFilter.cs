...
namespace ChocDino.UIFX
{
	...
	public class GlowFilter : FilterBase
	{
		...
		/// <summary>The resolution of the texture used by the gradient, ideally in powers of 2 size.</summary>
		public int GradientResolution { get { return _gradientResolution; } set { ChangeProperty(ref _gradientResolution, value); } }
		...
		[Tooltip("Amount to scale the noise (useful for hiding banding artifacts especially in dark regions). Set to zero to disable this shader feature.")]
		[SerializeField, Range(0f, 10f)] float _noiseScale = 1f;
		...
		/// <summary>Amount to scale the noise (useful for hiding banding artifacts especially in dark regions). Set to zero to disable this shader feature.</summary>
		public float NoiseScale { get { return _noiseScale; } set { ChangeProperty(ref _noiseScale, Mathf.Clamp(value, 0f, 10f)); } }
		...
		private GradientTexture _textureFromCurve;
		...
		private readonly static Vector2 s_maskSideBoth = new Vector2(1f, 0f);
		private readonly static Vector2 s_maskSideInside = new Vector2(0f, 1f);
		private readonly static Vector2 s_maskSideOutside = new Vector2(1f, -1f);
		...
		static new class ShaderProp
		{
			...
			public readonly static int MaskSide = Shader.PropertyToID("_MaskSide");
			public readonly static int SourceAlpha = Shader.PropertyToID("_SourceAlpha");
		}
		static class ShaderKeyword
		{
			public const string UseNoise = "USE_NOISE";
		...
		protected override void OnEnable()
		{
			...
			if (_fillMode == GlowFillMode.Gradient)
			{
				ChocDino.UIFX.GradientTexture.Create(ref _textureFromGradient, _gradientResolution);
			}
			Debug.Assert(_textureFromCurve == null);
			if (_falloffMode == GlowFalloffMode.Curve)
			{
				ChocDino.UIFX.GradientTexture.Create(ref _textureFromCurve, 128);
			}
		...
		protected override void OnDisable()
		{
			...
			ObjectHelper.Dispose(ref _textureFromCurve);
		...
		{
			// OnValidate is called when the scene is saved (and when prefab is saved), which causes the material (fields without properties) to lose their properties, so we force update them here.
			if (_blurfx != null)
			{
				_blurfx.ForceDirty();
			}
		...
		protected override void SetupDisplayMaterial(Texture source, Texture result)
		{
			...
			if (_noiseScale > 0f)
			{
				_displayMaterial.EnableKeyword(ShaderKeyword.UseNoise);
				_displayMaterial.SetFloat(ShaderProp.NoiseScale, _noiseScale);
			}
			else
			{
				_displayMaterial.DisableKeyword(ShaderKeyword.UseNoise);
			}
			...
			{
				case EdgeSide.Both:
					_displayMaterial.SetVector(ShaderProp.MaskSide, s_maskSideBoth);
					break;
				case EdgeSide.Inside:
					_displayMaterial.SetVector(ShaderProp.MaskSide, s_maskSideInside);
					break;
				case EdgeSide.Outside:
					_displayMaterial.SetVector(ShaderProp.MaskSide, s_maskSideOutside);
					break;
			}
