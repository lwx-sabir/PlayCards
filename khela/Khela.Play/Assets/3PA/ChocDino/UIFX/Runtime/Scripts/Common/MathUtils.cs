	...
	public static class MathUtils
	{
		...
		public static void LerpUnclamped(ref Matrix4x4 result, Matrix4x4 b, float t, bool preserveScale)
		{
			...
			if (preserveScale)
			{
				...
				// Clamp the value so we don't get division by zero - note we don't use Max() because we want to allow negative scale.
				if (scale.x == 0f) scale.x = 0.0001f;
				if (scale.y == 0f) scale.y = 0.0001f;
				if (scale.z == 0f) scale.z = 0.0001f;
		...
		/// <summary>
		/// Implementation of RectTransformUtility.PixelAdjustPoint()
		/// </summary>
		public static Vector2 PixelAdjustPoint(Vector2 point, RectTransform transform, Canvas canvas)
		{
			if (canvas == null || !canvas.pixelPerfect || canvas.renderMode == RenderMode.WorldSpace)
			{
				return point;
			}

			// 1. Convert local point to absolute Screen Space
			...
			// 2. Snap to the physical pixel grid
			Vector2 snappedScreenPoint = new Vector2(
				Mathf.Round(screenPoint.x),
				Mathf.Round(screenPoint.y)
			);

			// 3. Convert back to the local coordinate system of this transform
			RectTransformUtility.ScreenPointToLocalPointInRectangle(
				transform,
				snappedScreenPoint,
				canvas.worldCamera,
				out Vector2 localAdjustedPoint
			);

			return localAdjustedPoint;
		}

		/// <summary>
		/// Implementation of RectTransformUtility.PixelAdjustRect()
		/// </summary>
		public static Rect PixelAdjustRect(RectTransform transform, Canvas canvas)
		{
			if (canvas == null || !canvas.pixelPerfect || canvas.renderMode == RenderMode.WorldSpace)
			{
			...
			// 1. Convert local point to absolute Screen Space
			Vector3 worldPointMin = transform.TransformPoint(transform.rect.min);
			Vector3 worldPointMax = transform.TransformPoint(transform.rect.max);
			Vector2 screenPointMin = RectTransformUtility.WorldToScreenPoint(canvas.worldCamera, worldPointMin);
			Vector2 screenPointMax = RectTransformUtility.WorldToScreenPoint(canvas.worldCamera, worldPointMax);

			// 2. Snap to the physical pixel grid
			Vector2 snappedScreenPointMin = new Vector2(
				Mathf.Round(screenPointMin.x),
				Mathf.Round(screenPointMin.y)
			);
			Vector2 snappedScreenPointMax = new Vector2(
				Mathf.Round(screenPointMax.x),
				Mathf.Round(screenPointMax.y)
			);

			// TODO: new Unity implements some sort of scaling to make size the rectangle is always at least a pixel big.

			// 3. Convert back to the local coordinate system of this transform
			RectTransformUtility.ScreenPointToLocalPointInRectangle(
				transform,
				snappedScreenPointMin,
				canvas.worldCamera,
				out Vector2 localAdjustedPointMin
			);
			RectTransformUtility.ScreenPointToLocalPointInRectangle(
				transform,
				snappedScreenPointMax,
				canvas.worldCamera,
				out Vector2 localAdjustedPointMax
			);

			return Rect.MinMaxRect(localAdjustedPointMin.x, localAdjustedPointMin.y, localAdjustedPointMax.x, localAdjustedPointMax.y);
		}
		...
		public static Vector2 GetNormalizedAnchorPosition(TextAnchor anchor)
		{
