...
namespace ChocDino.UIFX
{
	...
	public class FilterStackTextMeshPro : UIBehaviour, IClippable
	{
		...
		private bool CanApplyFilter()
		{
			...
			if (string.IsNullOrEmpty(_textMeshPro.text)) return false;
			bool result = false;
			if (_filters != null)
			{
		...
		{
			if (!this.isActiveAndEnabled) return true;
			if (!GraphicComponent.enabled) return true;
			if (_filters != null)
			{
				// See if any filters make the output invisible
				foreach (var filter in _filters)
				{
					if (filter && filter.IsFiltered() && filter.DoParametersHideSource())
					{
						return true;
					}
				}
				...
				if (outlineWidth > 0f)
				{
					...
					else 
					{
						...
						if (_lastTextMaterialShader != material.shader)
						{
							...
							_lastMaterialShaderIsMobile = material.shader.name.StartsWith("TextMeshPro/Mobile", System.StringComparison.Ordinal);
						}
