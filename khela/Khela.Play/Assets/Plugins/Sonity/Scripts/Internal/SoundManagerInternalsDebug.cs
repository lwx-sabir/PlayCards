...
namespace Sonity.Internal {
    ...
    public class SoundManagerInternalsDebug {
        ...
        public bool logSoundEventsStop = false;
        public bool logSoundEventsPool = false;
        public bool logSoundEventsPause = false;
        public bool logSoundEventsUnpause = false;
        public bool logSoundEventsGlobalPause = false;
        public bool logSoundEventsGlobalUnpause = false;
        public bool logSoundEventsSoundParametersOnce = false;
        ...
        public void LogSoundEventsPlay(SoundEventBase soundEvent, SoundEventPlayType soundEventPlayType, Transform transformOwner, Transform transformPosition, Vector3? vector3Position) {
            ...
            if (logSoundEventsSelectObject == DebugSoundEventsToLogSelectObject.Owner) {
                ...
                } else if (soundEvent.internals.data.polyphonyMode == PolyphonyMode.LimitedPerOwner) {
                    selectedObject = transformOwner;
                }
            ...
            string transformOwnerName = "Null";
            if (selectedObject != null) {
                transformOwnerName = selectedObject.name;
            }
            if (soundEventPlayType == SoundEventPlayType.Play) {
                DebugLogPrint($"Sonity Log - <color=#62ff00>Play</color>: {soundEvent.name} at Owner \"{transformOwnerName}\"", selectedObject);
            } else if (soundEventPlayType == SoundEventPlayType.PlayAtVector) {
                DebugLogPrint($"Sonity Log - <color=#62ff00>Play</color>: {soundEvent.name} at Owner \"{transformOwnerName}\" and at Vector3 position {vector3Position.Value}", selectedObject);
            } else if (soundEventPlayType == SoundEventPlayType.PlayAtTransform) {
                string transformPositionName = "Null";
                if (transformPosition != null) {
                    transformPositionName = transformPosition.name;
                }
                DebugLogPrint($"Sonity Log - <color=#62ff00>Play</color>: {soundEvent.name} at Owner \"{transformOwnerName}\" and at Transform position \"{transformPositionName}\"", selectedObject);
            }
        }
