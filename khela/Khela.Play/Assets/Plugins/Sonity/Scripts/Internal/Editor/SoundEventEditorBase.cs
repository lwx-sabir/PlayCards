    ...
    public abstract class SoundEventEditorBase : Editor {
        ...
        private void GuiMuteSoloDisable() {
            ...
            if (muteEnable.boolValue) {
                // Red
                StartBackgroundColor(new Color(1f, 0f, 0f, EditorColorProSkin.GetCustomEditorBackgroundAlpha()));
            } else if (soloEnable.boolValue) {
                // Yellow
                StartBackgroundColor(new Color(1f, 1f, 0f, EditorColorProSkin.GetCustomEditorBackgroundAlpha()));
            } else if (disableEnable.boolValue) {
                // Magenta
                StartBackgroundColor(new Color(0.5f, 0f, 1f, EditorColorProSkin.GetCustomEditorBackgroundAlpha()));
            } else {
                // Transparent
                StartBackgroundColor(new Color(0f, 0f, 0f, 0f));
            }
        ...
        private void SelectSoundContainersInSoundEvents(List<SoundEventBase> soundEvents) {
            AssetDatabase.SaveAssets();
            ...
            for (int i = 0; i < soundEvents.Count; i++) {
                ...
                for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                    SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                    if (soundContainer != null && !soundContainers.Contains(soundContainer)) {
        ...
        private void GuiSpatializationInfo() {
            ...
            List<SoundEventBase> sounds3d = new List<SoundEventBase>();
            List<SoundEventBase> sounds2d = new List<SoundEventBase>();
            List<SoundEventBase> sounds3dNoDistance = new List<SoundEventBase>();
            List<SoundEventBase> sounds2dDistance = new List<SoundEventBase>();
            List<SoundEventBase> soundsMixed = new List<SoundEventBase>();
            List<SoundEventBase> soundsEmpty = new List<SoundEventBase>();

            for (int i = 0; i < mTargets.Length; i++) {
                SoundEventBase soundEvent = mTargets[i];
                if (soundEvent == null) {
                    continue;
                }
                bool sounds3dAdded = false;
                bool sounds2dAdded = false;
                bool sounds3dNoDistanceAdded = false;
                bool sounds2dDistanceAdded = false;

                for (int ii = 0; ii < mTargets[i].internals.soundContainers.Length; ii++) {
                    SoundContainerBase soundContainer = mTargets[i].internals.soundContainers[ii];
                    if (soundContainer == null) {
                        continue;
                    }
                    if (soundContainer.internals.data.spatialBlend > 0f && soundContainer.internals.data.distanceEnabled) {
                        if (!sounds3d.Contains(soundEvent)) {
                            sounds3d.Add(soundEvent);
                            sounds3dAdded = true;
                        }
                    } else if (soundContainer.internals.data.spatialBlend == 0f && !soundContainer.internals.data.distanceEnabled) {
                        if (!sounds2d.Contains(soundEvent)) {
                            sounds2d.Add(soundEvent);
                            sounds2dAdded = true;
                        }
                    } else if (soundContainer.internals.data.spatialBlend > 0f && !soundContainer.internals.data.distanceEnabled) {
                        if (!sounds3dNoDistance.Contains(soundEvent)) {
                            sounds3dNoDistance.Add(soundEvent);
                            sounds3dNoDistanceAdded = true;
                        }
                    } else if (soundContainer.internals.data.spatialBlend == 0f && soundContainer.internals.data.distanceEnabled) {
                        if (!sounds2dDistance.Contains(soundEvent)) {
                            sounds2dDistance.Add(soundEvent);
                            sounds2dDistanceAdded = true;
                        }
                    }
                }

                int soundTypeCountAdded = 0;
                ...
                if (sounds3dNoDistanceAdded) {
                    soundTypeCountAdded++;
                }
                if (sounds2dDistanceAdded) {
                    soundTypeCountAdded++;
                }

                // Sounds Mixed
                if (soundTypeCountAdded > 1) {
                    if (!soundsMixed.Contains(soundEvent)) {
                        soundsMixed.Add(soundEvent);
                    }
                    sounds3d.Remove(soundEvent);
                    sounds2d.Remove(soundEvent);
                    sounds3dNoDistance.Remove(soundEvent);
                    sounds2dDistance.Remove(soundEvent);
                }
                if (soundTypeCountAdded == 0) {
                    soundsEmpty.Add(soundEvent);
                }
            }
            ...
            if (soundsMixed.Count > 0) {
                multipleTypesCounter++;
            }
            if (soundsEmpty.Count > 0) {
                multipleTypesCounter++;
            }
            ...
            if (multipleTypesCounter == 0) {
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent("No SoundContainers"), GUILayout.Width(EditorGUIUtility.labelWidth));
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectAllSELabel, EditorTextSoundEvent.spatializationInfoSelectAllSETooltip))) {
            ...
            if (sounds3d.Count > 0) {
                EditorGUILayout.BeginHorizontal();
                if (sounds3d.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo3DSoundSingleLabel, EditorTextSoundEvent.spatializationInfo3DSoundTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo3DSoundMultipleLabel + " (" + sounds3d.Count + ")", EditorTextSoundEvent.spatializationInfo3DSoundTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = sounds3d.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectAllSELabel, EditorTextSoundEvent.spatializationInfoSelectAllSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        SelectObjectsOfSameType(false);
                    }
                    EndChange();
                }
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSCLabel, EditorTextSoundEvent.spatializationInfoSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    SelectSoundContainersInSoundEvents(sounds3d);
                }
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSet2DLabel, EditorTextSoundEvent.spatializationInfoSet2DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < sounds3d.Count; i++) {
                        SoundEventBase soundEvent = sounds3d[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                            if (soundContainer != null) {
            ...
            if (sounds2d.Count > 0) {
                EditorGUILayout.BeginHorizontal();
                if (sounds2d.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo2DSoundSingleLabel, EditorTextSoundEvent.spatializationInfo2DSoundTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo2DSoundMultipleLabel + " (" + sounds2d.Count + ")", EditorTextSoundEvent.spatializationInfo2DSoundTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = sounds2d.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
                ...
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSCLabel, EditorTextSoundEvent.spatializationInfoSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    SelectSoundContainersInSoundEvents(sounds2d);
                }
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSet3DLabel, EditorTextSoundEvent.spatializationInfoSet3DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < sounds2d.Count; i++) {
                        SoundEventBase soundEvent = sounds2d[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                            if (soundContainer != null) {
            ...
            if (sounds3dNoDistance.Count > 0) {
                ...
                if (sounds3dNoDistance.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo3DSoundWithoutDistanceSingleLabel, EditorTextSoundEvent.spatializationInfo3DSoundWithoutDistanceTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo3DSoundWithoutDistanceMultipleLabel + " (" + sounds3dNoDistance.Count + ")", EditorTextSoundEvent.spatializationInfo3DSoundWithoutDistanceTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = sounds3dNoDistance.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
                ...
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSCLabel, EditorTextSoundEvent.spatializationInfoSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    SelectSoundContainersInSoundEvents(sounds3dNoDistance);
                }
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSet2DLabel, EditorTextSoundEvent.spatializationInfoSet2DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < sounds3dNoDistance.Count; i++) {
                        SoundEventBase soundEvent = sounds3dNoDistance[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                            if (soundContainer != null) {
                                Undo.RecordObject(soundContainer, "Set 2D and Disable Distance");
                                soundContainer.internals.data.spatialBlend = 0f;
                                soundContainer.internals.data.distanceEnabled = false;
                                EditorUtility.SetDirty(soundContainer);
                            }
            ...
            if (sounds2dDistance.Count > 0) {
                ...
                if (sounds2dDistance.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo2DSoundWithDistanceSingleLabel, EditorTextSoundEvent.spatializationInfo2DSoundWithDistanceTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfo2DSoundWithDistanceMultipleLabel + " (" + sounds2dDistance.Count + ")", EditorTextSoundEvent.spatializationInfo2DSoundWithDistanceTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = sounds2dDistance.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
                ...
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSCLabel, EditorTextSoundEvent.spatializationInfoSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    SelectSoundContainersInSoundEvents(sounds2dDistance);
                }
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSet3DLabel, EditorTextSoundEvent.spatializationInfoSet3DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < sounds2dDistance.Count; i++) {
                        SoundEventBase soundEvent = sounds2dDistance[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                            if (soundContainer != null) {
                                Undo.RecordObject(soundContainer, "Set 3D and Enable Distance");
                                soundContainer.internals.data.spatialBlend = 1f;
                                soundContainer.internals.data.distanceEnabled = true;
                                EditorUtility.SetDirty(soundContainer);
                            }
            ...
            if (soundsMixed.Count > 0) {
                EditorGUILayout.BeginHorizontal();
                if (soundsMixed.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfoMixedSpatializationLabel, EditorTextSoundEvent.spatializationInfoMixedSpatializationTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfoMixedSpatializationLabel + " (" + soundsMixed.Count + ")", EditorTextSoundEvent.spatializationInfoMixedSpatializationTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = soundsMixed.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
                ...
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSCLabel, EditorTextSoundEvent.spatializationInfoSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    SelectSoundContainersInSoundEvents(soundsMixed);
                }
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSet3DLabel, EditorTextSoundEvent.spatializationInfoSet3DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < soundsMixed.Count; i++) {
                        SoundEventBase soundEvent = soundsMixed[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                            if (soundContainer != null) {
                                Undo.RecordObject(soundContainer, "Set 3D & Enable Distance");
            ...
            if (soundsEmpty.Count > 0) {
                EditorGUILayout.BeginHorizontal();
                if (soundsEmpty.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfoEmptyLabel, EditorTextSoundEvent.spatializationInfoEmptyTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfoEmptyLabel + " (" + soundsEmpty.Count + ")", EditorTextSoundEvent.spatializationInfoEmptyTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = soundsEmpty.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
            ...
            if (multipleTypesSelected) {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfoEditAllLabel, EditorTextSoundEvent.spatializationInfoEditAllTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoAllSelectSCLabel, EditorTextSoundEvent.spatializationInfoAllSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    List<SoundEventBase> soundEvents = new List<SoundEventBase>();
                    soundEvents.AddRange(sounds3d);
                    soundEvents.AddRange(sounds2d);
                    soundEvents.AddRange(sounds3dNoDistance);
                    soundEvents.AddRange(sounds2dDistance);
                    SelectSoundContainersInSoundEvents(soundEvents);
                }
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoAllSet3DLabel, EditorTextSoundEvent.spatializationInfoAllSet3DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < mTargets.Length; i++) {
                        SoundEventBase soundEvent = mTargets[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
                ...
                EndChange();
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoAllSet2DLabel, EditorTextSoundEvent.spatializationInfoAllSet2DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < mTargets.Length; i++) {
                        SoundEventBase soundEvent = mTargets[i];
                        for (int ii = 0; ii < soundEvent.internals.soundContainers.Length; ii++) {
                            SoundContainerBase soundContainer = soundEvent.internals.soundContainers[ii];
