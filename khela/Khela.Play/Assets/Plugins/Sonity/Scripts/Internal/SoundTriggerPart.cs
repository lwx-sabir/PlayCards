...
namespace Sonity.Internal {
    ...
    public class SoundTriggerPart {
        ...
        public void Stop(Transform owner, bool allowFadeOut) {
            if (!SoundManagerIsNull()) {
                SoundManagerBase.Instance.Internals.Stop(soundEvent, owner, OwnerPlayType.Normal, allowFadeOut);
            }
        }
        ...
        public void Pause(Transform owner, bool forcePause) {
            if (!SoundManagerIsNull()) {
                SoundManagerBase.Instance.Internals.Pause(soundEvent, owner, OwnerPlayType.Normal, forcePause);
            }
        }

        public void Unpause(Transform owner) {
            if (!SoundManagerIsNull()) {
                SoundManagerBase.Instance.Internals.Unpause(soundEvent, owner, OwnerPlayType.Normal);
            }
        }
