    ...
    public class SoundManagerInternals {
        ...
        private void AwakeSetupPreloadVoices() {
                ...
                if (settings.voicePreload > 0 && !settings.asyncStartup) {
                    Debug.LogError($"Sonity.{nameof(NameOf.SoundManager)}: Steam Audio in combination with Voice Preload requires Async Startup.");
                }
#endif
        ...
        public void ManagedUpdate() {
            ...
            while (managedUpdateTempInstanceDictionaryValueEnumerator.MoveNext()) {
                ...
                // To Delete SoundEvent Instances (unloaded by eg addressables)
        ...
        public void InternalStop(SoundEventBase soundEvent, Transform owner, OwnerPlayType ownerPlayType, bool allowFadeOut) {
            switch (ownerPlayType) {
                ...
                case OwnerPlayType.Music:
                    ...
                    if (soundEventDictionary.TryGetValue(soundEvent.GetAssetID(), out tempInstanceDictionaryValue)) {
                        if (tempInstanceDictionaryValue.instanceDictionary.TryGetValue(SoundManagerBase.Instance.Internals.cachedMusicTransform.GetInstanceID(), out tempInstance)) {
                            tempInstance.PoolAllVoices(allowFadeOut, true, false);
                            return;
                        }
                    }
                ...
                case OwnerPlayType.UI:
                    ...
                    if (soundEventDictionary.TryGetValue(soundEvent.GetAssetID(), out tempInstanceDictionaryValue)) {
                        if (tempInstanceDictionaryValue.instanceDictionary.TryGetValue(SoundManagerBase.Instance.Internals.cachedUITransform.GetInstanceID(), out tempInstance)) {
                            tempInstance.PoolAllVoices(allowFadeOut, true, false);
                            return;
                        }
                    }
...
#else
            Debug.LogError($"Sonity.{nameof(NameOf.SoundManager)}: The Script Define Symbol \"SONITY_ENABLE_ADDRESSABLE_AUDIOMIXER\" is false but you're trying to access the Addressable AudioMixer");
            return null;
#endif
        }
    }
