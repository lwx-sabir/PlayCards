    ...
    public abstract class SoundManagerBase : MonoBehaviour {
        ...
        void ScriptingDefineSymbolsWarnings()
        {
            ...
            if (!internals.settings.volumeIncreaseEnable) {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Enable Volume Increase\" is disabled but the Script Define Symbol \"SONITY_ENABLE_VOLUME_INCREASE\" exists.", Instance);
            }
#else
            if (internals.settings.volumeIncreaseEnable)
            {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Enable Volume Increase\" is enabled but the Script Define Symbol \"SONITY_ENABLE_VOLUME_INCREASE\" is not added.", Instance);
            }
#endif
#if SONITY_ENABLE_ADDRESSABLE_AUDIOMIXER
            if (!internals.settings.addressableAudioMixerUse) {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Addressable AudioMixer\" is disabled but the Script Define Symbol \"SONITY_ENABLE_ADDRESSABLE_AUDIOMIXER\" exists.", Instance);
            }
#else
            if (internals.settings.addressableAudioMixerUse)
            {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Addressable AudioMixer\" is enabled but the Script Define Symbol \"SONITY_ENABLE_ADDRESSABLE_AUDIOMIXER\" is not added.", Instance);
            }
#endif
#if SONITY_ENABLE_INTEGRATION_STEAM_AUDIO
            if (!Instance.internals.settings.steamAudioIntegrationEnable) {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Steam Audio Integration\" is disabled but the Script Define Symbol \"SONITY_ENABLE_INTEGRATION_STEAM_AUDIO\" exists.", Instance);
            }
#else
            if (Instance.internals.settings.steamAudioIntegrationEnable)
            {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Steam Audio Integration\" is enabled but the Script Define Symbol \"SONITY_ENABLE_INTEGRATION_STEAM_AUDIO\" is not added.", Instance);
            }
#endif
#if SONITY_ENABLE_INTEGRATION_PLAYMAKER
            if (!Instance.internals.settings.playmakerIntegrationEnable) {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"PlayMaker Integration\" is disabled but the Script Define Symbol \"SONITY_ENABLE_INTEGRATION_PLAYMAKER\" exists.", Instance);
            }
#else
            if (Instance.internals.settings.playmakerIntegrationEnable)
            {
                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"PlayMaker Integration\" is enabled but the Script Define Symbol \"SONITY_ENABLE_INTEGRATION_PLAYMAKER\" is not added.", Instance);
            }
#endif
//#if SONITY_ENABLE_INTEGRATION_ENTITY_SOUND
//            if (!Instance.internals.settings.entitySoundIntegrationEnable) {
//                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Entity Integration\" is disabled but the Script Define Symbol \"SONITY_ENABLE_INTEGRATION_ENTITY_SOUND\" exists.", Instance);
//            }
//#else
//            if (Instance.internals.settings.entitySoundIntegrationEnable)
//            {
//                Debug.LogWarning($"Sonity.{nameof(NameOf.SoundManager)}: \"Entity Integration\" is enabled but the Script Define Symbol \"SONITY_ENABLE_INTEGRATION_ENTITY_SOUND\" is not added.", Instance);
//            }
//#endif
