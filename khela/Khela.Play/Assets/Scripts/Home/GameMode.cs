...
namespace PlayCard.Home
{
    ...
    /// Lobby buttons. The carousel tells it when it's the centred one via <see cref="SetSelected"/> (only the
    /// centred game shows its buttons). Button onClick is wired here in code (not the inspector) and routes by
    /// the definition — so you assign the title/buttons ONCE on the prefab and every instance works; only
    /// <see cref="definition"/> (and the 3D table) vary per instance.
    /// </summary>
    public sealed class GameMode : MonoBehaviour
    {
        ...
        /// <summary>Carousel calls this on every selection change — only the focused mode shows its title +
        /// buttons; the buttons additionally require the game to be available.</summary>
