...
var Lib_BEST_HTTP_WebGL_WS_Bridge =
{
	...
	WS_Create: function(url, protocol, onOpen, onText, onBinary, onError, onClose, allocator)
	{
		...
		socket.socketImpl.onmessage = function (e)
		{
			...
			else // Text
			{
				...
				var content = HEAPU8.subarray(array);
				#if USE_PTHREADS
				    content = new Uint8Array(content);
				#endif
				
				const ret = encoder.encodeInto(e.data, content);
	...
	WS_Send_String: function (id, ptr, pos, length)
	{
        ...
        var buff = HEAPU8.subarray ? HEAPU8.subarray(startPtr, endPtr) : new Uint8Array(HEAPU8.slice(startPtr, endPtr));
        
        #if USE_PTHREADS
            // Make a copy from the content
            buff = new Uint8Array(buff);
        #endif
        
        var str = UTF8Decoder.decode(buff);
	...
	WS_Send_Binary: function(id, ptr, pos, length)
	{
		...
		try
		{
			...
			#if USE_PTHREADS
			    socket.socketImpl.send(new Uint8Array(buff));
			#else
